<?php
// admin/api/vender.php (VERSIÓN MULTIVENTA + ANOTACIONES)
...
try {
    ...
    // AQUÍ SE AGREGÓ LA COLUMNA "anotaciones"
    $sql_venta = "INSERT INTO ventas (
        ...
        tipo_pasajero, origen_boleto, destino_boleto, total_pagado, canal, oficina_venta, estado, anotaciones
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'CONFIRMADO', ?)";
    ...
    foreach ($pasajeros as $pax) {
        ...
        $anotacion = $pax['anotaciones'] ?? '';

        $stmt_venta->execute([
            $codigo_ticket, $id_viaje, $fecha_historica, $bus_historico,
            $pax['asiento'], $pax['rut'], $pax['nombre'], $pax['telefono'], 
            $pax['tipo_pasajero'], $origen, $destino, $pax['total_pagado'], $canal, $oficina, $anotacion
        ]);
